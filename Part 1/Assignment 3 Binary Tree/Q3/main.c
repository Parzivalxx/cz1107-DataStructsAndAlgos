...

{
    ...
    if (node == NULL)
        return 0;
    if((node->left) && (node->right)) //have 2 children
        return countOneChildNodes(node->left) + countOneChildNodes(node->right);
    if ((node->left == NULL) && (node->right != NULL)) //have right child
        return 1 + countOneChildNodes(node->right);
    if ((node->right == NULL) && (node->left != NULL)) //have left child
        return 1 + countOneChildNodes(node->left);
    return 0; //no children, leaf node
}
