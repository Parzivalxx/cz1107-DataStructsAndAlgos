...
ListNode* HashSearch(HashTable Q3Hash, int key)
{
    ...
    if (Q3Hash.hSize==0||Q3Hash.nSize==0)
        return NULL;
...
int HashInsert(HashTable* Q3HashPtr, int key)
{
    ...
    if (temp == NULL) {
        Q3HashPtr->Table[index].head = (ListNode*)(malloc(sizeof(ListNode)));
        Q3HashPtr->Table[index].head->key = key;
        Q3HashPtr->Table[index].head->next = NULL;
    }
    else {
