...
void sumToC(LinkedList* ll, int C, ArrayList* al)
{
    ...
    if (ll->sum == C) {
        ArrayNode *itertmp, *arrtmp;
        int *num = (int *)malloc((ll->size)*sizeof(int));
        arrtmp = (ArrayNode*) malloc(sizeof(ArrayNode));
        temp = ll->head;
        for (int i = 0; i < ll->size; i++) {
            *(num + i) = temp->item;
            temp = temp->next;
        }
        arrtmp->itemArray = num;
        arrtmp->sizeArray = ll->size;
        arrtmp->next = NULL;
        if (al->size == 0)
            al->head = arrtmp;
        else {
            itertmp = al->head;
            while ((itertmp->next) != NULL)
    ...
    if (ll->size > 0) {
        last = findNode(*ll, (ll->size)-1)->item;
//        if (last == C)
//            return;
    }
...
//            insertNode(&tmpll, k, findNode(*ll, k)->item);
        sumToC(ll, C, al);
